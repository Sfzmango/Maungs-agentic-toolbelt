...
This repo is a project-agnostic, human-gated **agentic software-development workflow engine** — a carpenter's toolbelt of developer agents and skills that helps one developer move like a whole team (the force multiplier behind a 10x developer; not just a hammer, the whole belt to ship) — that runs on multiple agent CLIs rather than one, and is itself the single canonical source those targets are generated from, not an application. It ships **16 agents + 11 skills (27 components)**: agents are `@name` subagents (specialized workers) and skills are `/name` conductors (orchestrators). Together they take a piece of work from a raw idea to a security-reviewed, merge-ready PR, and keep a codebase's docs current. Every component auto-detects the **host** project's stack and conventions at runtime — nothing is hardcoded to one language or framework. It targets **Claude Code** first/primary, ships a port for the **OpenAI Codex CLI**, has a **Gemini CLI** port in progress, and accommodates others (e.g. the DeepSeek CLI) as future targets via the generator seam. On Claude Code it is distributed two ways: as a plugin via the `maung-tools` marketplace, and via a copy/symlink `install.sh` into `~/.claude`; on the Codex CLI via `codex plugin add` plus `install-codex.sh` (see `docs/codex.md`).
...
- **No added attribution** in commit messages, PR bodies, or files. This repo's
  owner-only instruction forbids human or automated contributor attribution; the
  project-agnostic shipped guard mechanically denies AI-attributed commits
  without imposing the owner-only rule on consumer repositories.
  *(`CONTRIBUTING.md`, `hooks/pretooluse-guard.sh`)*
...
- **Every PR bumps the version (CI hard-fail):** the plugin is distributed by `.claude-plugin/plugin.json` `version` and Claude Code caches installs by it, so CI (`validate.yml` check 13) **fails any PR whose version does not strictly increase** vs. the base branch — every PR, no exceptions. Bump `.claude-plugin/plugin.json` **and** keep `plugins/maungs-agentic-toolbelt/.codex-plugin/plugin.json` in **parity** (strict semver `MAJOR.MINOR.PATCH`; `validate_codex` enforces parity). The comparison is `tools/check_version_bump.py` (unit-tested by `tests/test_version_bump.py`). This rule is for developing **this** repo (the plugin) — it is NOT imposed on host projects the toolbelt operates on (a future opt-in flag may offer it to consumers). *(`.github/workflows/validate.yml` check 13)*
...
- **Owner-only attribution** — no co-author trailers, contributor credits,
  collaborator credits, contributor lists, or generated-by footers. Preserve only
  the existing owner copyright/license attribution. *(`CONTRIBUTING.md`; AI
  attribution is additionally enforced by `hooks/pretooluse-guard.sh`)*
...
- **Bump the version every PR** — strictly increase `.claude-plugin/plugin.json` `version` and keep the Codex manifest (`plugins/maungs-agentic-toolbelt/.codex-plugin/plugin.json`) in parity; CI hard-fails otherwise (see Gotchas). Patch for fixes/docs, minor for a new component/feature. *(`.github/workflows/validate.yml` check 13)*
