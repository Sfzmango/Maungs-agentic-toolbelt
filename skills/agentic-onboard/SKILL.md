...
description: "Prepare an existing repo for agentic development. Detect COLD repos with no agent context and STALE repos whose context has drifted; build one verified project profile; delegate CLAUDE.md, AGENTS.md, and architecture-context writing to @context-writer; use @context-auditor for drift; and show diffs before changing existing files. Default scope is lean; --deep also hands off to /wiki-generator. Use for onboarding a repo, generating or refreshing agent instructions, or repairing stale project context. Writes the working tree only and never commits or pushes. Invoke as /agentic-onboard or /agentic-onboard --deep --target all."
