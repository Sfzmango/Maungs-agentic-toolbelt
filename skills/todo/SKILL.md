...
Then read `$file` if it exists (it won't on first use). Also read the project's `CLAUDE.md` only for an attribution/terminology convention — never to decide *what* is on the list.
