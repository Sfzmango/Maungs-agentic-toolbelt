...
- **Set up:** `git fetch origin <default-branch>` then `git worktree add -b chore/<slug> <tmp-path> origin/<default-branch>` (a path under the system temp dir, never inside the repo).
