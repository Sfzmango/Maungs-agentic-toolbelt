...
6. **Honor target-project conventions** from `CLAUDE.md` / `CLAUDE.local.md` — the project's commit convention, its existing CHANGELOG style, its versioning scheme, its domain terminology. Match an existing `CHANGELOG.md` format rather than imposing a new one.
...
1. **Agent-context docs** — `CLAUDE.md` + `CLAUDE.local.md`. Inherit the project's commit convention, versioning scheme, domain terminology, and any "release notes look like X" rule. If an existing `CHANGELOG.md` is present, infer and match its style.
...
Honor any existing CHANGELOG style and the project's CLAUDE.md conventions throughout.
