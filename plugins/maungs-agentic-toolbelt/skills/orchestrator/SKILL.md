...
5. **Honor project-specific conventions from `AGENTS.md (and CLAUDE.md when present)` + `CLAUDE.local.md`.** Project rules are non-negotiable for the agents working in that project.

## Codex status line

Codex provides a built-in `/statusline` command for standard footer fields such as model, context, limits, git state, tokens, and session. It does not consume the Claude toolbelt's custom pipeline-status file, so do not write `~/.codex/toolbelt-status.json`.

## Auto-detection on every invocation

Before running any phase, detect:

1. **`AGENTS.md (and CLAUDE.md when present)` + `CLAUDE.local.md`** — agent-context files. If present, agents inherit project cardinal rules.
...
If the project lacks agent-context discipline (no AGENTS.md (and CLAUDE.md when present), no plan files, no roadmap), distinguish two cases before starting:
...
- **Empty / near-empty tree (greenfield)** — when ALL THREE hold: no recognized package manifest, no `AGENTS.md (and CLAUDE.md when present)`/`AGENTS.md`, and fewer than a small threshold (~5) of tracked source files (`git ls-files | wc -l` is near-zero) — this is a from-scratch build. Route it into discovery: pass **`--greenfield`** to the architect (`spawn architect subagent plan topic "<text>" --greenfield`) so its **Phase 0 — Discovery** runs the structured foundation interview, rather than falling through to the generic defaults branch. All three signals are required so a real-but-sparse repo doesn't false-fire; the architect's approval gate is the backstop. The user may also force this explicitly with `$orchestrator --greenfield <topic>` (see the "`--greenfield` flag" section).
...
3. **MCP servers** — `codex mcp list`. Look for:
   ...
   - a **playwright** server → provides the `playwright` MCP server for `spawn developer subagent`'s live UI verification (**optional**).
...
4. **AGENTS.md (and CLAUDE.md when present)** — from auto-detection above. Absent ⇒ recommend scaffolding one (the highest-leverage thing for agent quality).

**Classify each gap and act accordingly:**

| Class | Examples | What you do |
|---|---|---|
| **A — auto-fixable** | add an MCP server (`codex mcp add …`) | Propose it behind ONE explicit confirmation gate that shows the EXACT command; run only after the user approves. NEVER run an install / `mcp add` / `npx` command silently. |
| **B — needs the user** | `gh auth login`, creating/pasting a token, approving a permission prompt, **starting a new Codex thread** | Print the exact copy-paste command/steps and PAUSE. You cannot do these for the user. |
| **C — optional** | Playwright MCP | Offer it; if declined, proceed and note that `spawn developer subagent`'s live browser verification will be skipped for this run. |
...
# Playwright MCP — optional, only for developer subagent's live UI verification:
...
**Critical — newly added MCP servers may require a new Codex thread.** After adding any server, STOP and tell the user to start a new thread and re-run `$orchestrator <arg>`; Step 0 will re-check and pass. Do NOT pretend a just-added server is usable in the current thread.
...
Optionally invoke `spawn product-owner subagent refine issue <num>` if the issue body is sparse.

### Step 3 — Delegate to architect subagent

```
spawn architect subagent plan issue <num>    # or spawn architect subagent plan topic "<text>"
```

If the tree is empty/near-empty or `--greenfield` was passed, append `--greenfield` to the invocation so the architect's Phase 0 — Discovery foundation interview runs (`spawn architect subagent plan topic "<text>" --greenfield`).

Wait for return. Capture: plan file path, PR number (architect opens the PR with commit #1).

### Steps 4-6 — Handled inside architect subagent

Planning + plan approval + branch + commit #1 + open PR are all inside the architect's invocation.

### Steps 7-9 — Delegate to developer subagent

```
spawn developer subagent implement plan <plan-file-path>
```

Wait for return. Capture: commit #2 SHA, local gate output, any plan corrections.

### Step 10 — Delegate to pr-reviewer subagent

```
spawn pr-reviewer subagent PR <num>
```
...
- "Apply punch list, then ship" → loop to Step 7-9 (fix iteration via developer subagent)
- "I'll write feedback in chat" → wait
- "Abort"

Fix loop: max 3 iterations. Iteration 4+ → mandatory handoff.

**11b — Ready-to-merge sub-gate**: when reviewer happy + user picks "Ship as-is", ask the user in chat and wait for an explicit “yes” / confirmation before proceeding (never interpret silence as approval):
...
```
spawn developer subagent plan-sync <plan-file-path> PR <num>
```
...
### Step 13 — Delegate to resolution subagent

```
spawn resolution subagent PR <num>
```
...
- The orchestrator passes `--greenfield` straight through to the architect: `spawn architect subagent plan topic "<text>" --greenfield`.
- The orchestrator ALSO infers greenfield automatically from the empty-tree heuristic (no manifest + no `AGENTS.md (and CLAUDE.md when present)`/`AGENTS.md` + <~5 tracked source files) — the flag is the explicit override for the cases the heuristic is intentionally conservative about.
...
If a project has its own `$orchestrator` skill (e.g., `<project>/.agents/skills/orchestrator/SKILL.md`), the project version takes precedence when invoked inside that project. This global skill is the fallback for any project that doesn't define its own.
