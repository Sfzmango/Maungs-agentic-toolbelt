...
The argument is in `the invocation arguments from the user's request`. It selects the MODE and the FORMAT (see below). If `the invocation arguments from the user's request` contains an unrecognized flag, echo it and stop — do not guess intent.
...
## MODES (from `the invocation arguments from the user's request`)
...
6. **Honor target-project conventions** from `AGENTS.md (and CLAUDE.md when present)` / `CLAUDE.local.md` — the project's commit convention, its existing CHANGELOG style, its versioning scheme, its domain terminology. Match an existing `CHANGELOG.md` format rather than imposing a new one.
...
1. **Agent-context docs** — `AGENTS.md (and CLAUDE.md when present)` + `CLAUDE.local.md`. Inherit the project's commit convention, versioning scheme, domain terminology, and any "release notes look like X" rule. If an existing `CHANGELOG.md` is present, infer and match its style.
...
Honor any existing CHANGELOG style and the project's AGENTS.md (and CLAUDE.md when present) conventions throughout.
