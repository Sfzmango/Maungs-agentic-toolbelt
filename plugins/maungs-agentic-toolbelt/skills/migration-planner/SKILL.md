...
description: "Read-only pre-flight for RISKY data/schema migrations. Given a proposed schema change — described in prose, or an existing migration file passed in the invocation arguments from the user's request — it produces a MIGRATION RISK DOSSIER BEFORE anything is written or run. It NEVER writes or runs the migration: it analyzes affected tables/columns + row counts, DATA-LOSS risks (drops, type narrowing, NOT NULL on existing data), LOCK / DOWNTIME risks flagged by the detected database, a BACKFILL strategy, the EXPAND/CONTRACT (parallel-change) zero-downtime rollout, a concrete ROLLBACK plan, and the blast radius of code that reads/writes the affected schema. Auto-detects the migration system + ORM (ActiveRecord, Prisma, Alembic, Flyway, Liquibase, Django, Knex, sqlx, …) and the database. Then, only behind a human gate, hands off to developer subagent / $chore to implement. Invoke as `$migration-planner <description>` or `$migration-planner <path/to/migration-file>`."
---
...
The argument is in `the invocation arguments from the user's request`. It is either:
...
If `the invocation arguments from the user's request` is empty, ask what change to analyze and stop — never guess the target. If `the invocation arguments from the user's request` contains an unrecognized flag, echo it and stop.
...
These hold for every invocation in every project. The target project's `AGENTS.md (and CLAUDE.md when present)` may ADD conventions (its migration safety rules, its zero-downtime policy, its forbidden operations); it never removes these:
...
4. **Human-gated handoff — never auto-implement.** You may RECOMMEND handing off to `spawn developer subagent` (full pipeline) or `$chore` (chore-sized), but you do not invoke them and you do not write the migration yourself. The handoff happens only after an explicit human gate (see HANDOFF GATE). Suggest, never auto-run.
5. **Honor project-specific migration conventions from `AGENTS.md (and CLAUDE.md when present)` / `CLAUDE.local.md`** — the project's zero-downtime policy, its "no `DOWN` migrations" or "every migration must be reversible" rule, its forbidden operations, its strong-migrations/online-DDL tooling. Project rules are non-negotiable and override your generic defaults where stricter.
...
1. **Agent-context docs** — `AGENTS.md (and CLAUDE.md when present)` + `CLAUDE.local.md` (or equivalent). Inherit the project's migration safety rules, zero-downtime policy, forbidden operations, tenancy/scoping rules, and domain terminology. A large share of "is this migration safe?" is already answered by a rule written here.
...
If the project lacks agent-context discipline (no `AGENTS.md (and CLAUDE.md when present)`, no documented migration policy), surface that before producing the dossier and proceed with conservative, engine-default-safe assumptions — explicitly labelled as assumptions.
...
1. **CHANGE SUMMARY** — one paragraph, plain language: what schema change is proposed and why (from `the invocation arguments from the user's request`). Echo the parsed target so the human can catch a wrong table/column.
2. **DETECTED CONTEXT** — migration system + ORM, database engine + version (or "undetermined — analyzed per-engine"), schema source of truth, and any governing `AGENTS.md (and CLAUDE.md when present)` migration rules you're honoring.
...
12. **RECOMMENDED HANDOFF** — `spawn developer subagent` (full pipeline, for a multi-deploy EXPAND/CONTRACT) vs. `$chore` (for a genuinely trivial, no-data, no-lock additive change), with a one-line rationale. This is a recommendation only — it fires only through the HANDOFF GATE below.

## LEAST-PRIVILEGE TOOL SCOPING

This skill is read-only analysis. The scope it operates within:

- **Read / Grep / Glob** — read schema files, migrations, models, config, `AGENTS.md (and CLAUDE.md when present)`; grep the blast radius. Primary tools.
...
- **Downstream agents** (`spawn developer subagent`, `$chore`) are *recommended*, never invoked by this skill — they fire only after the human passes the HANDOFF GATE.
...
- After delivering the dossier, ask the human (e.g. by asking the user in chat (wait for an explicit “yes” / confirmation before proceeding; never interpret silence as approval)): **"Hand this off to implement, or stop here?"** with options:
  - **"Hand off to `spawn developer subagent`"** — for a phased EXPAND/CONTRACT rollout (multi-deploy). You surface the recommended invocation string for the human; you do NOT invoke it yourself.
...
| **`the invocation arguments from the user's request` points at an already-APPLIED migration** | Note it's already applied; pivot to a "what did this do + is the live schema/code consistent + is a corrective migration needed" analysis rather than a pre-flight. Do not propose re-running it. |
...
| **`AGENTS.md (and CLAUDE.md when present)` migration rule conflicts with a generic recommendation** | The project rule wins; align the dossier to it and note the override. (CARDINAL RULE 5.) |
