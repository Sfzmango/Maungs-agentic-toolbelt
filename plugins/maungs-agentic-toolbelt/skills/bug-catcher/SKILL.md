...
1. **`AGENTS.md / CLAUDE.md` + `CLAUDE.local.md`** — agent-context files. If present, they carry the project's cardinal rules, documented gotchas, and the dominant catastrophic bug classes. Read them; the gotcha list is your highest-yield bug catalog.
...
If the project lacks this discipline (no AGENTS.md / CLAUDE.md, no plan files), proceed with sensible defaults and say so up front.
...
- It does **not** touch load-bearing content — anything CI asserts (e.g. component-count strings), `AGENTS.md / CLAUDE.md` / `AGENTS.md`, or a link **target** / numeric value / prose meaning.
