---
name: "bug-catcher"
description: "Find, verify, and triage bugs in any project, then hand an approved fix plan to $orchestrator (or $chore if it's chore-sized). DELEGATES diagnosis to the bug-catcher-rick subagent and refutation to the bug-catcher-adversary subagent. Default mode debugs ONE specifically-described bug through three phases — bug-catcher-rick subagent diagnoses the root cause with an evidence chain, a cold-eyes bug-catcher-adversary subagent tries to refute it, the conductor runs a bounded debate, then writes an approved fix plan and hands off. The `--global` flag instead sweeps the ENTIRE codebase + test suites + any browser/E2E playbooks + the project's documentation/Markdown (broken mermaid, rendering and link defects), adversarially verifies every candidate bug, and produces a severity-ranked backlog with per-bug plans — and may auto-fix a purely-cosmetic doc defect (e.g. an un-renderable mermaid block) via a bounded `$chore --concurrently --bypass`. Invoke as `$bug-catcher <symptom>` or `$bug-catcher --global`."
---

# $bug-catcher — catch the bug, prove it's real, hand off a fix

You are the **conductor**. You do not diagnose or refute yourself — you delegate diagnosis to the `spawn bug-catcher-rick subagent` and the adversarial double-check to the `spawn bug-catcher-adversary subagent`, run the debate between them, and own the human gates + handoff. The pattern mirrors `$orchestrator`: named agents do the work in their own context windows; you sequence them and talk to the developer.
...
1. **`AGENTS.md (and CLAUDE.md when present)` + `CLAUDE.local.md`** — agent-context files. If present, they carry the project's cardinal rules, documented gotchas, and the dominant catastrophic bug classes. Read them; the gotcha list is your highest-yield bug catalog.
...
6. **The project's memory context directory** (Codex-injected memories, when enabled) — captured feedback and known-issue history.

If the project lacks this discipline (no AGENTS.md (and CLAUDE.md when present), no plan files), proceed with sensible defaults and say so up front.

The argument is in `the invocation arguments from the user's request`. Parse the mode first:
...
- **otherwise** → run **Mode A** (targeted) on the bug described in `the invocation arguments from the user's request`.
- **`the invocation arguments from the user's request` empty and no `--global`** → ask what to debug (a symptom, a failing test, an error message, a PR number) and stop.
...
4. **No fabricated root causes.** If `spawn bug-catcher-rick subagent` returns a labelled HYPOTHESIS (not CONFIDENT) and `spawn bug-catcher-adversary subagent` can't confirm it, that's an escalation, not a handoff.
...
### Phase 1 — Diagnose (delegate to bug-catcher-rick subagent)

```
spawn bug-catcher-rick subagent <the symptom / failing test / error / PR number from the invocation arguments from the user's request>
```

The agent reproduces or locates the failure, separates symptom from cause, checks it against the documented gotchas in the project's `AGENTS.md (and CLAUDE.md when present)`, and returns a **bug dossier** (symptom · reproduction · root cause [CONFIDENT|HYPOTHESIS] · evidence chain @ file:line · fix direction · regression test · blast radius · prod-mitigation note · open questions).
...
### Phase 2 — Adversarial double-check + debate (delegate to bug-catcher-adversary subagent)

```
spawn bug-catcher-adversary subagent <the full dossier from Phase 1>
```
...
- **DISPUTED / WRONG-ROOT-CAUSE / INCONCLUSIVE** → hand the adversary's findings back to `spawn bug-catcher-rick subagent` for a fresh diagnosis pass (re-spawn it with the original symptom + the adversary's critique), then re-spawn `spawn bug-catcher-adversary subagent` with the revised dossier + the prior exchange so it sees what changed and why. Cap the debate at **3 rounds**.
...
`--global` was passed. Instead of one bug, sweep the whole surface — code, tests, E2E playbooks, and the project's documentation/Markdown — for *all* of them, verify each adversarially, and produce a ranked backlog with plans (auto-fixing only purely-cosmetic doc defects; see below). This is a large fan-out; for a thorough sweep, use a finder→verify pipeline (a `spawn bug-catcher-rick subagent` finder stage → `spawn bug-catcher-adversary subagent` verify stage) — invoking `--global` is the developer's explicit opt-in to multi-agent orchestration. For a quick pass, parallel agent spawns are fine. Tell the developer up front roughly how wide you're going so the cost isn't a surprise.

### Phase 1 — Fan-out discovery

Spawn one `spawn bug-catcher-rick subagent` finder per slice, in parallel, each blind to the others so coverage doesn't collapse onto one angle. Derive the slices from what the project actually has; the list below is a starting template — adapt to the detected language/framework:
...
Deduplicate candidates across slices (same root cause surfacing in multiple files = one bug). Then run **every surviving candidate through `spawn bug-catcher-adversary subagent`** exactly as in Mode A Phase 2 — the verdict decides whether it's a real bug. **Drop refuted/INCONCLUSIVE candidates from the backlog** (or list them separately as "unconfirmed, needs evidence") — a global report full of false positives is worse than a short true one. Note explicitly anything you bounded or sampled rather than exhaustively checked; no silent truncation.
...
- It does **not** touch load-bearing content — anything CI asserts (e.g. component-count strings), `AGENTS.md (and CLAUDE.md when present)` / `AGENTS.md`, or a link **target** / numeric value / prose meaning.
