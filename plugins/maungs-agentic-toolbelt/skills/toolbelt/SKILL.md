...
The argument is in `the invocation arguments from the user's request`. Four modes, dispatched by what `the invocation arguments from the user's request` contains:

- **(A) Inventory** — empty `the invocation arguments from the user's request`: print the stage-grouped INVENTORY of every installed component.
...
- **(C) Status** — `the invocation arguments from the user's request` is exactly `status`: a read-only environment check (router, MCP servers, AGENTS.md (and CLAUDE.md when present), installed components).
- **(D) Metrics** — `the invocation arguments from the user's request` is exactly `metrics`: a read-only summary of the usage telemetry (how often the toolbelt was offered vs actually used).

Dispatch the reserved keywords `status` and `metrics` to modes C and D BEFORE treating `the invocation arguments from the user's request` as a free-text goal (mode B). If `the invocation arguments from the user's request` contains an unrecognized flag (anything starting `--`), echo it and stop — do not guess intent.

## Purpose

Be the single discoverable entry point now that the toolbelt has grown to ~27 components (16 agents + 11 skills + hooks). New users do not know that `$bug-catcher` exists, or whether to reach for `$orchestrator` vs `$chore`, or that a prompt-router is quietly suggesting things. This skill answers three questions and nothing more: **what is installed**, **which one fits my goal**, and **is my environment ready**. It is the only component whose job is the toolbelt itself rather than a target project — so it must stay accurate to the REAL installed set (derive it; don't hardcode a list that rots) and it must stay strictly read-only.

## CARDINAL RULES (refuse to violate)

These hold for every invocation. A target project's `AGENTS.md (and CLAUDE.md when present)` may add conventions but never removes these:

1. **READ-ONLY, always.** This skill never writes, edits, commits, pushes, opens/merges PRs, adds MCP servers, installs anything, or mutates settings. The ONLY commands you may run are read-only probes: `ls`, `git` reads (`git rev-parse`, `git status`, `git config --get`), `codex mcp list`, and reading files under the installed plugin/skills/agents dirs. If a probe would mutate state, do not run it.
2. **Never invoke a component on the user's behalf.** You may print the exact invocation string (`$orchestrator 42`, `spawn developer subagent implement plan <path>`) — you do NOT execute it. The user (or another agent) chooses to run it. Recommending is the whole job; running is theirs. This is the front door, not the elevator.
3. **Stay accurate to the real component set — derive, don't hardcode.** Build the inventory by reading plugin `skills/*/SKILL.md` frontmatter and installed `~/.codex/agents/*.toml` metadata (`name` + `description`) where you can reach them. A hardcoded list silently rots as components are added/removed. If you cannot reach the install dir, fall back to the known set but SAY you fell back, so the human knows the list may be stale.
...
6. **Honor target-project conventions** from `AGENTS.md (and CLAUDE.md when present)` / `CLAUDE.local.md` if present (terminology, "do not surface X"). Project rules are non-negotiable.
...
3. **Installed agents** — enumerate `~/.codex/agents/*.toml`; read each file's `name` + `description` and sandbox/MCP settings. Source of truth for the AGENTS half.
...
5. **Target-project context** — is there a `AGENTS.md (and CLAUDE.md when present)` (case-insensitive) in the current working directory's repo? Its absence is the single highest-leverage thing to flag (recommend `$agentic-onboard`).
6. **Environment signals** (status mode, read-only):
   ...
   - **MCP servers** — `codex mcp list`: look for a **github** server (the `github` MCP server, needed by the issue/PR-driven flows) and a **playwright** server (the `playwright` MCP server, optional, for live UI verification). You can also tell from your own live toolset whether those tools are present this session.
...
|---|---|---|
| **Onboarding** | Prep a repo for agentic work; build/refresh agent-context | `$agentic-onboard`, `spawn context-writer subagent`, `spawn context-auditor subagent` |
| **Conductor** | The orchestration entry point that delegates to agents | `$orchestrator` |
| **Plan** | Turn an ask into a scoped issue + a vetted plan | `spawn product-owner subagent`, `spawn architect subagent`, `spawn plan-reviewer subagent` |
| **Build** | Implement an approved plan with the gates | `spawn developer subagent` |
| **Review** | Fresh-eyes correctness + security gates on a PR | `spawn pr-reviewer subagent`, `spawn security-reviewer subagent`, `spawn security-mentor subagent` |
| **Wrap-up** | Resolve review threads; merge-ready hygiene | `spawn resolution subagent` |
| **Bug** | Diagnose + adversarially verify defects | `$bug-catcher`, `spawn bug-catcher-rick subagent`, `spawn bug-catcher-adversary subagent` |
...
| **Wiki** | Generate + maintain a technical wiki | `$wiki-generator`, `spawn wiki-writer subagent`, `spawn wiki-auditor subagent` |
...
2. **Match** the goal to the best-fit component(s) by intent. Use the same intent map the prompt-router uses (it is the maintained mapping), applied to the detected set. Representative mappings:
   ...
   | "build/add/ship a feature" / "implement an issue" | `$orchestrator <issue-id\|topic>` | Full plan→build→review→merge-ready cycle, human-gated at every commit/push. Add `spawn architect subagent` first if scope is fuzzy. |
   | "plan/design/scope this" / "write an issue" / "RFC" | `spawn architect subagent` and/or `spawn product-owner subagent` | Architect front-loads decisions into a vetted plan; product-owner turns a fuzzy ask into a scoped issue with acceptance criteria. |
   | "review this PR" / "is this correct/safe?" | `spawn pr-reviewer subagent PR <n>` (+ `spawn security-reviewer subagent PR <n>` if security matters) | Fresh-eyes correctness/quality verdict; security gate maps to SOC2/OWASP/PCI/NIST. |
   ...
   | "prep/onboard this repo" / "no AGENTS.md (and CLAUDE.md when present)" / "make it agent-ready" | `$agentic-onboard` (add `--deep` for a full wiki) | Generates the agent-context files the rest of the toolbelt depends on; handles cold + stale repos. |
   ...
   | "table this" / "remind me later" / "add to my backlog" | `$todo <text>` | Private, per-project backlog for tabled work — stored locally, never committed; `$todo` alone lists it. It only records the item; you run the work later. |
   | "teach me the security finding" | `spawn security-mentor subagent PR <n>` | Same review as the gate but explains the threat model + fix per finding. |
...
| **GitHub MCP** | `codex mcp list` + own live the `github` MCP server toolset | `CONNECTED` / `NOT CONNECTED` |
| **Playwright MCP** | `codex mcp list` + own live the `playwright` MCP server toolset | `CONNECTED` / `NOT CONNECTED (optional)` |
| **AGENTS.md (and CLAUDE.md when present) (current repo)** | file presence in the repo root | `PRESENT` / `MISSING → suggest $agentic-onboard` |
...
| **Components installed** | enumerate plugin `skills/*` + `~/.codex/agents/*.toml` | counts, e.g. `11 skills + 16 agents (+ hooks router)` |

For each gap, print the read-only consequence and the user-owned remediation — but **do not run the remediation**. Examples:
- GitHub MCP `NOT CONNECTED` ⇒ "the issue/PR-driven flows (`$orchestrator` on an issue ID, the review/resolution agents) need it; add it with `codex mcp add … github …` then start a new Codex thread." Print the command for the user to run; you do not run it (CARDINAL RULE 1).
- `AGENTS.md (and CLAUDE.md when present) MISSING` ⇒ "run `$agentic-onboard` to generate it — most components lean on it."
- Newly-added MCP servers may require a new Codex thread; if you observe a server in `codex mcp list` but its tools are absent from your live toolset, report it as `CONFIGURED — new thread required`, not `CONNECTED`.

Status is purely informational. It changes nothing.

## Mode D — Metrics (`$toolbelt metrics`)

A read-only summary of the toolbelt's own usage telemetry: how often the prompt-router **offered** a component, how often an agent/skill was actually **invoked**, and how often a suggestion **converted** into a run in the same session. This is opt-in and off by default — the hooks only record when `MAUNGS_TOOLBELT_DEBUG` is `on` (or `verbose`, which also traces each event to stderr). Records land in an append-only JSONL log on the user's machine (`~/.codex/maungs-toolbelt/usage.jsonl`, override with `MAUNGS_TOOLBELT_LOG`) — never inside a project repo.
...
1. Locate it (in order): `the bundled scripts/toolbelt-metrics.sh next to this skill`, else the `bin/toolbelt-metrics.sh` next to the detected install root.
2. Run it read-only: `bash "<skill-dir>/scripts/toolbelt-metrics.sh"`. It needs `jq`. It only READS the log — it never writes, edits, or deletes (consistent with CARDINAL RULE 1).
3. Print its output. If telemetry is OFF or the log is empty, the script already prints the enable instructions — relay them.
...
| **`codex mcp list` errors or is unavailable** (status mode) | Report MCP checks as `UNKNOWN — could not query`, not `NOT CONNECTED`. Distinguish "couldn't check" from "checked and absent". |
...
> `$toolbelt`
Detect the installed set → group by stage → print the inventory (one line per component with purpose + how to invoke) → footer pointing at `$toolbelt <goal>` and `$toolbelt status`. Flag a missing `AGENTS.md (and CLAUDE.md when present)`. Nothing is executed.
...
> `$toolbelt status`
Read-only check: router `ACTIVE`/`DISABLED`/`NOT INSTALLED`, GitHub + Playwright MCP `CONNECTED`/`NOT CONNECTED`, `AGENTS.md (and CLAUDE.md when present)` present?, git identity set?, telemetry `RECORDING`/`OFF`, and the installed component counts — each with the user-owned remediation for any gap. Changes nothing.
