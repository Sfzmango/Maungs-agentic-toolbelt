...
- **(C) Status** — `$ARGUMENTS` is exactly `status`: a read-only environment check (router, MCP servers, AGENTS.md / CLAUDE.md, installed components).
...
6. **Honor target-project conventions** from `AGENTS.md / CLAUDE.md` / `CLAUDE.local.md` if present (terminology, "do not surface X"). Project rules are non-negotiable.
...
5. **Target-project context** — is there a `AGENTS.md / CLAUDE.md` (case-insensitive) in the current working directory's repo? Its absence is the single highest-leverage thing to flag (recommend `@agentic-onboard`).
...
5. **Surface gaps**: if `AGENTS.md / CLAUDE.md` is absent in the current repo, add ONE line recommending `@agentic-onboard` first. If you fell back to the documented set, say so.
...
2. **Match** the goal to the best-fit component(s) by intent. Use the same intent map the prompt-router uses (it is the maintained mapping), applied to the detected set. Representative mappings:
   ...
   | "prep/onboard this repo" / "no AGENTS.md / CLAUDE.md" / "make it agent-ready" | `@agentic-onboard` (add `--deep` for a full wiki) | Generates the agent-context files the rest of the toolbelt depends on; handles cold + stale repos. |
...
| **AGENTS.md / CLAUDE.md (current repo)** | file presence in the repo root | `PRESENT` / `MISSING → suggest @agentic-onboard` |
...
| **Components installed** | enumerate `skills/*` + `agents/*` | counts, e.g. `11 skills + 16 agents (+ hooks router)` |

For each gap, print the read-only consequence and the user-owned remediation — but **do not run the remediation**. Examples:
- GitHub MCP `NOT CONNECTED` ⇒ "the issue/PR-driven flows (`@orchestrator` on an issue ID, the review/resolution agents) need it; add it with `codex mcp add … github …` then restart your agent." Print the command for the user to run; you do not run it (CARDINAL RULE 1).
- `AGENTS.md / CLAUDE.md MISSING` ⇒ "run `@agentic-onboard` to generate it — most components lean on it."
- Newly-added MCP servers do not load until Claude Code restarts; if you observe a server in `codex mcp list` but its tools are absent from your live toolset, report it as `CONFIGURED — restart required`, not `CONNECTED`.
...
> `@toolbelt`
Detect the installed set → group by stage → print the inventory (one line per component with purpose + how to invoke) → footer pointing at `@toolbelt <goal>` and `@toolbelt status`. Flag a missing `AGENTS.md / CLAUDE.md`. Nothing is executed.
...
> `@toolbelt status`
Read-only check: router `ACTIVE`/`DISABLED`/`NOT INSTALLED`, GitHub + Playwright MCP `CONNECTED`/`NOT CONNECTED`, `AGENTS.md / CLAUDE.md` present?, git identity set?, telemetry `RECORDING`/`OFF`, and the installed component counts — each with the user-owned remediation for any gap. Changes nothing.
