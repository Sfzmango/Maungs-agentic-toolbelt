...
This is a reference index of all **26 components** that make up the pipeline: **16 agents** (invoked via `@name`) and **10 skills** (invoked via `/name`). Components are grouped by the stage of the dev cycle they serve.
...
Some patterns are not a component of their own — they are conventions shared across components. These are docs, not agents or skills, so they do not change the **16 agents + 9 skills** count.
