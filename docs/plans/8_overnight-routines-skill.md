...
Each enabled job is **always its own cloud routine** (`--bug` / `--security` / `--wiki` select; default = all three; two flags ⇒ two routines), staggered a few minutes apart. They condense into **ONE rolling issue** titled `[overnight] Dossier` (per repo) **without a shared mutable body** — because `gh issue edit --body` is a *full-body overwrite* with no per-section atomicity, and the bug run alone can take 20–40 min and overlap the others, three routines editing one body would lost-update each other. Instead: the issue **body** is a static index + `🚨 Review first` skeleton **seeded once** on first setup and never rewritten by a routine; **each job owns ONE comment** tagged with a hidden marker (`<!-- overnight:bug -->` / `:security` / `:wiki`) and **only ever edits THAT comment** (`gh issue comment` create-or-edit), leading its comment with its own criticals (bug SEV1/SEV2; security compliance-blockers/P0s). Because every routine touches only its own comment, they **cannot clobber each other** — the per-comment edit is the race-safe coordination primitive (replacing the unsound shared-section-body design the first draft proposed). With one job selected there is just the one job-comment.
