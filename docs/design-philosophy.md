...
This pipeline is **16 agents + 11 skills (27 components)** that take a fuzzy ask all the way to a merged PR — plus side-flows for bug diagnosis, code translation, chores, handoffs, and a self-maintaining codebase wiki. Agents are invoked with `@name` and run with a scoped toolset; skills are invoked with `/name` and act as conductors that delegate to agents.

It is built as a **carpenter's toolbelt**, not a single power tool: a conductor to route, a planner and a builder to do the work, fresh-eyes reviewers and a security gate to check it, and bug/wiki/chore/handoff side-flows for the work that doesn't fit the main line — each a purpose-built tool with a least-privilege scope, not one general-purpose blade asked to do everything. Not a hammer, but the whole belt — the belt is what ships the job, not any single tool on it. And because the components are defined once and rendered per target, the same belt buckles onto whichever agent CLI you work in: Claude Code today, the OpenAI Codex CLI now, the Gemini CLI next.

What makes it a *system* rather than a bag of prompts is that the same eight design principles recur in every component. They were not retrofitted into a style guide after the fact — each one is a response to a specific, observed failure mode of LLM-driven development (context contamination, scope creep, silent quality regression, runaway token spend, an agent quietly committing on your behalf). This document states each principle, the failure it defends against, and points at the concrete place in the components where it is enforced.
...
## 8. Shared declarative interview catalogs, not a runtime interviewer middleware

**The principle.** When several components need the same kind of guaranteed *question coverage* — a from-scratch build needs its stack/deploy/MVP foundation; a risky migration needs its change-shape/data-state/rollback facts — the coverage is delivered as a **shared, declarative interview catalog**: a named markdown checklist of bucketed questions a consuming component walks *before* its main work. It is explicitly **not** delivered as a centralized runtime "interviewer middleware" that would sit between every agent and the user, own a shared interview state, and route answers across agents. The catalogs live in one home, [`docs/interview-catalogs/`](interview-catalogs/) — an index/README defining the pattern plus one file per catalog — and each consuming component **embeds the catalog's buckets** at its discovery phase (so it is self-contained at runtime) **and** points at the canonical file.

**The failure it defends against.** Two, actually. First, the obvious one: foundational questions that ride on the model's general competence get skipped under load — a greenfield build that never asks "container or serverless?" pays for it mid-implementation, and the architect's standing lesson is that a mid-build pivot costs ~10× the same question asked during planning. A guaranteed checklist closes that gap. Second, the subtler one the *middleware* would have introduced: a shared cross-agent interview-state store is exactly the kind of hidden context channel that breaks the fresh-eyes split (§3) — it is the mechanism by which a reviewer that is supposed to be blind quietly learns what an upstream agent concluded. The declarative catalog keeps the one thing the middleware got right (coverage) and discards the coupling that would have undermined the rest of the system.

**Why the middleware was declined.** A middleware would intercept all agent↔user questioning and centralize the interview. It was rejected for four reasons, by weight: **(1) no clean interception point** — subagents run in isolated Task contexts with no shared message bus, and the harness's `AskUserQuestion` is already the native, uniform questioning UI a middleware would merely reimplement with less integration; **(2) it fights fresh-eyes (§3)** — a shared interview-state store is the cross-agent context channel that risks leaking into the context-blind reviewers; **(3) it centralizes what the architecture distributes** — least-privilege (§2) and per-agent ownership of gates (§4) are core house style, and a middleware re-couples them; **(4) the cross-agent dedup it would buy already exists** — the orchestrator carries answered decisions forward through the plan file + issue body, and agents re-read those at their phase boundary, so an agent doesn't re-ask what an upstream agent already settled.

**The six invariants** every catalog obeys: **(1) declarative** — data, not code, with no runtime or shared state; **(2) bucketed** — questions grouped so one `AskUserQuestion` call maps onto one or two buckets; **(3) coverage rule** — every line is asked or explicitly marked `n/a — <reason>`, never silently skipped; **(4) front-loaded** — walked *before* the component's main deliverable (the 10× lesson, generalized); **(5) referenced, not centralized** — consumers embed the buckets and point at the canonical file, with nothing sitting between agent and user; **(6) drift-guarded** — a CI check asserts each catalog's bucket headers appear in its consuming component(s), the same family as the component-count drift guards.

**Concrete examples.**

- **Instance #1 — greenfield discovery**, consumed by `@architect`'s **Phase 0 — Discovery** ([`docs/interview-catalogs/greenfield.md`](interview-catalogs/greenfield.md)). Gated on a greenfield signal (an explicit `--greenfield` flag or a strict empty-tree heuristic — no manifest *and* no `CLAUDE.md`/`AGENTS.md` *and* <~5 tracked files), it walks four buckets (product framing · technical foundation · delivery & ops · constraints & scaffolding) and writes the answers into a `## Foundation` plan section plus a minimal `CLAUDE.md` seed — the one case where the ecosystem *creates* the context it normally consumes.
- **Instance #2 — migration discovery**, consumed by `/migration-planner` ([`docs/interview-catalogs/migration.md`](interview-catalogs/migration.md)), walked *before* the risk dossier so the dossier rests on answered facts (change shape · data state · lock & downtime · rollout · rollback · blast radius) rather than assumptions.
- **The drift guard makes "referenced, not centralized" enforceable.** Because each consumer embeds the buckets, the copy in the agent and the canonical file could drift; CI asserts every catalog's bucket headers appear in its declared consumer(s), so a renamed bucket or an orphaned catalog fails the build — the same enforcement family as the count-string checks.
- **Catalogs earn their keep.** Only the two with a real, observed coverage gap ship; future candidates (a bug-repro catalog for `/bug-catcher`, a security-scoping catalog for `@security-reviewer`) are added only when a gap is observed — the pattern is open-ended, not a mandate to pre-build every checklist.

## How the principles reinforce each other

These are not eight independent rules; they interlock, and that interlock is the actual engineering argument:
...
- **Interview catalogs (§8) complete auto-detection (§1) at the one place it has nothing to detect** — a greenfield tree — and do it *without* breaking fresh-eyes (§3). Choosing a declarative catalog over a runtime middleware was precisely a choice to add coverage as data rather than as a shared-state channel that would have leaked into the context-blind reviewers; the drift guard that keeps catalog and consumer in sync is the same enforcement family as the count-string checks, so §8 inherits the rest of the system's discipline rather than bolting on a parallel one.
